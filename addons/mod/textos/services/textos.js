...
.factory('$mmaModPage', function($mmFilepool, $mmSite, $mmFS, $http, $log, $q, $mmSitesManager, $mmUtil, mmaModPageComponent) {
    ...
    /**
     * Gets the textos HTML.
     *
     * @module mm.addons.mod_textos
     * @ngdoc method
     * @name $mmaModPage#getTextosHtml
     * @param {Object} contents The module contents.
     * @param {Object} moduleId The module ID.
     * @return {Promise}
     */
    self.getTextosHtml = function(contents, moduleId) {
        var indexUrl,
            paths = {},
            promise;
        ...
        angular.forEach(contents, function(content) {
            ...
            if (self._isMainPage(content)) {
                // This seems to be the most reliable way to spot the index textos.
                indexUrl = url;
            } else {
        ...
        promise = (function() {
            ...
            if (!indexUrl) {
                ...
                $log.debug('Could not locate the index textos');
                return $q.reject();
    ...
    /**
     * Returns whether the file is the main textos of the module.
     *
     * @module mm.addons.mod_textos
     * @ngdoc method
    ...
    self._isMainPage = function(file) {
        var filename = file.filename || undefined,
            ...
            url = '/mod_textos/content/index.html',
    ...
    /**
     * Check if textos plugin is enabled in a certain site.
     *
     * @module mm.addons.mod_textos
     * @ngdoc method
    ...
    /**
     * Report a textos as being viewed.
     *
     * @module mm.addons.mod_textos
     * @ngdoc method
    ...
    self.logView = function(id) {
        if (id) {
            ...
            return $mmSite.write('mod_textos_view_textos', params);
        }
        return $q.reject();
    };
